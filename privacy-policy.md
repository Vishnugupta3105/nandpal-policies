```markdown
# Privacy Policy for Nandpal

**Last updated:** November 5, 2024

## Introduction
Nandpal ("we," "our," or "us") respects your privacy. This policy explains how we collect, use, and protect your information.

## Information We Collect

### Personal Information
- Name and age (for personalization)
- Phone number (for account creation)
- Gender (optional, for personalized experience)

### Usage Data
- Chanting sessions and counts
- Village structures built
- Tasks completed
- Virtue progress
- Chat conversations with Krishna AI

### Device Information
- Device type and model
- Operating system version
- App version
- Crash reports and diagnostics

## How We Use Your Information

We use your information to:
- Provide personalized spiritual guidance
- Track your progress and achievements
- Sync data across devices
- Improve app features
- Provide Krishna AI responses
- Send important notifications

## Data Storage and Security

- All data is encrypted in transit (HTTPS)
- Stored securely on cloud servers
- We do not sell your data to third parties
- We do not use your data for advertising

## Your Rights

You have the right to:
- Access your personal data
- Request data deletion
- Export your data
- Opt out of analytics
- Disable notifications

## Children's Privacy

Our app is suitable for all ages. We do not knowingly collect personal information from children under 13 without parental consent.

## Third-Party Services

We use:
- Cloudinary (for media storage)
- Render (for backend hosting)
- Google Analytics (for app analytics)

## Data Retention

We retain your data as long as your account is active. You can request deletion at any time.

## Changes to Privacy Policy

We may update this policy. Changes will be posted in the app and on this page.

## Contact Us

For privacy concerns or data requests:
- Email: support@nandpal.app
- In-app: Settings > Help & Support

---

By using Nandpal, you agree to this Privacy Policy.
```
